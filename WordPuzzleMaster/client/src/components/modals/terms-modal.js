    ...
    return (<Dialog open={open} onOpenChange={(open) => !open && onDecline()}>
      <DialogContent className="bg-card/90 border-accent/30 p-6 m-4 max-w-lg w-full">
        ...
        <div className="flex justify-center gap-4">
          ...
          <Button variant="outline" className="bg-card hover:bg-background/80 text-white font-medium py-2 px-6 rounded-full transition-all magic-btn" onClick={onDecline}>
            Decline
          </Button>
        </div>
      </DialogContent>
