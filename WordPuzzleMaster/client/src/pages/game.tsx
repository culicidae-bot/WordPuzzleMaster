...
// Update the import path below if the schema file is located elsewhere, or create the file if missing.
...
export default function Game() {
  ...
  const handleWordFound = (word: string) => {
    ...
    if (level && level.words.some((w: { text: string }) => w.text === word) && !foundWords.includes(word)) {
  ...
  const handleUseHint = () => {
    ...
    const remainingWords = level.words
      ...
      .filter((word: string) => !foundWords.includes(word));
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-between max-w-2xl mx-auto w-full px-4 pb-6">
        ...
        <LetterCircle 
          ...
          onToggleMultiplayer={() => setShowMultiplayer(true)} hintsRemaining={0}        />
      </main>
