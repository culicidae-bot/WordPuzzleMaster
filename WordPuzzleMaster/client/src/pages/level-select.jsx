...
export default function LevelSelect() {
    ...
    const userLevel = ((_a = gameState.user) === null || _a === void 0 ? void 0 : _a.currentLevel) || 1;
    useEffect(() => {
        const fetchLevels = () => __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield apiRequest("/api/levels");
    ...
    return (<div className="min-h-screen pt-6 pb-16 px-4">
      <div className="max-w-4xl mx-auto">
        <Button className="mb-6 text-white" onClick={() => navigate("/")}>
        ...
        {loading ? (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {[...Array(8)].map((_, i) => (<Card key={i} className="aspect-square bg-card/50 animate-pulse">
                <CardContent className="p-0 h-full"></CardContent>
              </Card>))}
          </div>) : (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {levels.map((level) => (<Card key={level.id} className={`relative overflow-hidden cursor-pointer transition-transform hover:scale-105 ${level.id <= userLevel
                    ? "bg-card/70"
                    : "bg-card/30 grayscale"}`} onClick={() => handleLevelSelect(level)}>
                <CardContent className="p-0 h-full flex flex-col items-center justify-center py-6">
                  {level.id > userLevel && (<div className="absolute inset-0 flex items-center justify-center bg-black/40">
                  ...
                  <div className="flex gap-1">
                    {[...Array(3)].map((_, i) => (<Star key={i} className={`h-5 w-5 ${i < (level.id <= userLevel ? 3 : 0)
